...
export default function Max8TrainerPage() {
  const positions = ["UTG", "MP", "LJ", "HJ", "CO", "BTN", "SB", "BB"];
  
  const seatCoords = [
    { left: "50%", top: "72%" }, // 0: Bottom Center (Hero)
    { left: "18%", top: "68%" }, // 1: Bottom Left
    { left: "8%", top: "38%" },  // 2: Left
    { left: "28%", top: "10%" }, // 3: Top Left
    { left: "50%", top: "5%" },  // 4: Top Center
    { left: "72%", top: "10%" }, // 5: Top Right
    { left: "92%", top: "38%" }, // 6: Right
    { left: "82%", top: "68%" }  // 7: Bottom Right
  ];
  ...
  useEffect(() => {
    setIsClient(true);
    const saved = localStorage.getItem("pokerTrainerStats8Max");
    if (saved) {
  ...
  function getAdvice(position: string, hand: any, scenario: any, stack: number) {
    ...
    if (scenario.level === 0) {
      ...
      return { action: "fold", title: "Fold correcto", reason: `${hand.code} es demasiado débil para abrir desde ${position} en una mesa de 8 jugadores.` };
    }
  ...
  function handleAction(action: string) {
    ...
    setStats(newStats);
    localStorage.setItem("pokerTrainerStats8Max", JSON.stringify(newStats));
  ...
  function resetStats() {
    ...
    setStats(s);
    localStorage.setItem("pokerTrainerStats8Max", JSON.stringify(s));
  }
  ...
  return (
    <div className="custom-spin-trainer">
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .seat { position: absolute; width: 104px; min-height: 76px; transform: translate(-50%, -50%); background: rgba(0,0,0,0.72); border: 1px solid rgba(255,255,255,0.10); border-radius: 16px; padding: 8px; z-index: 5; text-align: center; transition: 0.25s ease; }
        ...
        .avatar { width: 30px; height: 30px; border-radius: 50%; margin: 0 auto 5px; background: radial-gradient(circle at 30% 20%, #fff, transparent 18%), linear-gradient(135deg, #2a2a2a, #050505); border: 1px solid rgba(212,175,55,0.45); display: grid; place-items: center; font-size: 13px; }
        .seat .name { font-size: 11px; color: #fff; font-weight: 800; margin-bottom: 2px; }
        .seat .pos { font-size: 11px; color: var(--gold-2); font-weight: 900; letter-spacing: 1px; }
        .seat .stack { margin-top: 4px; font-size: 11px; color: var(--muted); }
        .dealer-btn { position: absolute; right: -6px; top: -6px; width: 22px; height: 22px; border-radius: 50%; display: grid; place-items: center; background: #fff; color: #111; font-size: 11px; font-weight: 900; box-shadow: 0 2px 5px rgba(0,0,0,0.5); }
      ...
      <main className="spin-app">
        <header className="spin-header">
          ...
          <h1>Poker Trainer Visual Max 8</h1>
          <p>
            Entrena tus decisiones preflop en un formato de mesa 8-Max.
          ...
          <div className="poker-area">
            <div className="poker-table">
              <div className="table-logo">
                ...
                <div className="sub">Preflop Lab 8-Max</div>
              </div>
